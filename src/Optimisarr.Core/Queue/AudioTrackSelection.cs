...
public static class AudioTrackSelection
{
    ...
    public static bool TryNormaliseLanguageList(string? value, out string? normalised)
    {
        ...
        if (codes.Count == 0 || codes.Any(code => !IsAsciiLanguageCode(code, 2, 3)))
        {
            return false;
        }
    ...
    private static bool IsUnknown(string? tag)
    {
        ...
        if (UnknownTags.Contains(code) || !IsAsciiLanguageCode(code, 2, 3))
        {
            return true;
        }
    ...
    private static bool IsAsciiLanguageCode(string value, params int[] lengths) =>
        lengths.Contains(value.Length) && value.All(char.IsAsciiLetter);
}
