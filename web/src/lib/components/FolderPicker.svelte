...
>
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto p-2">
      {#if loading}
        <p class="p-4 text-center text-sm text-ink-4">{i18n.m.common.loading_short}</p>
      {:else if error}
        <p class="p-4 text-center text-sm text-bad">{error}</p>
      {:else if listing && listing.directories.length > 0}
        ...
        {/each}
      {:else}
        <p class="p-4 text-center text-sm text-ink-4">{i18n.m.shared.no_subfolders}</p>
      {/if}
    </div>
