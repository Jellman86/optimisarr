...
<article class="working-job card" class:compact class:selected aria-label={title}>
  <div class="working-poster"><Thumbnail mediaFileId={job.mediaFileId} size={compact ? 'md' : 'poster'} /></div>
  ...
  <div class="working-footer">{#if !compact}<span>{job.status === 'Leased' ? job.workerName : i18n.m.dashboard.this_server}</span>{/if}<button id={`working-job-${job.id}`} class="btn btn-ghost" aria-expanded={selected} onclick={onopen}>{i18n.m.queue.view_job}<Icon name="arrow-right" /></button></div>
</article>
