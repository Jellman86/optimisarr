...
//
// The navigation used to be seven words with no numbers on them, so "is there anything in
// quarantine?" meant opening quarantine to find out. One poll of /api/stats answers all of it,
// and it is a call the dashboard already makes, so the sidebar costs one request every fifteen
// seconds rather than one per page.
import { api } from '../api'
