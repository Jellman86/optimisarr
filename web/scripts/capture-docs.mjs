...
try {
 ...
 for(const [room,name]of[['encoding','settings-general'],['files','settings-files'],['media-servers','settings-connections'],['download-managers','settings-downloads'],['notifications','settings-notifications'],['workers','settings-workers'],['system','settings-system']]){await go('/settings/'+room);await expect(page.getByRole('button',{name:'All settings',exact:true})).toBeVisible();await shot(name)}
