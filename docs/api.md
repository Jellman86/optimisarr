...
| `GET` | `/api/workers` | List paired workers with an `online` flag the server computes from its own liveness rule, `activeJobs` (each with `stage` and `progress`), and `lastProblem` — the most recent thing the server refused or discarded from that worker: a lapsed lease, a candidate from the wrong source or with the wrong hash, a delivered candidate that failed verification. Never returns credential fingerprints. |
