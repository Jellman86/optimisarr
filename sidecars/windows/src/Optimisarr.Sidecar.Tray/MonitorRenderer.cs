...
internal static class MonitorRenderer
{
    public static void Render(string directory)
    {
        ...
        foreach (var state in new[] { "idle", "encoding", "verifying", "offline", "light-encoding", "details", "light-details", "preferences", "light-preferences", "preview-fallback", "light-preview-fallback", "two-jobs", "light-two-jobs" })
        {
            ...
            var jobs = pose is "encoding" or "verifying" or "details" or "preview-fallback" or "two-jobs"
            ...
            window.ProcessingDetails.IsExpanded = pose is "details" or "two-jobs";
            if (pose == "preferences")
            {
